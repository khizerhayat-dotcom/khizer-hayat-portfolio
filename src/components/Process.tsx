        ...
        <div className="mt-12 grid gap-10 sm:mt-14 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          {STEPS.map((step, i) => (
            <Reveal key={step.number} delay={i * 0.08} className="relative">
              <span className="font-display text-5xl font-bold tracking-normal text-ink/10 dark:text-white/10">
                {step.number}
              </span>
              <h3 className="mt-4 font-display text-lg font-bold tracking-normal text-ink dark:text-white">
                {step.title}
              </h3>
